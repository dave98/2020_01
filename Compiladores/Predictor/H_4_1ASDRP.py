...
class CATgrammar:
    ...
    def create_classes_carpet(self):
        ...
        print("\tRedactando no terminales")
        file = open("d_non_terminal.py", "w+")
        file.write("from AbstractExpression import *\n")
        file.write("from d_terminal import * \n\n")
        for non_terminal in self.non_terminals:
            avoid_duplicates = [];
            if not non_terminal.isalnum(): # In non terminlas this is useless
            ...
            file.write("\t\t\tcontext.TakeActualContext()\n")
            file.write("\t\t\tif len(childs) > 0:\n")
            file.write("\t\t\t\tchilds.reverse()\n")
            file.write("\t\t\t\tfor child in childs:\n")
            file.write("\t\t\t\t\tcontext.SetMoreContext(child)\n")
            file.write("\t\telse:\n")
            file.write("\t\t\tprint(\"Interpreter fail\")\n")
            file.write("\t\treturn context\n\n")

        file.close()


        print("\tRedactando terminales")
        file = open("d_terminal.py", "w+")
        file.write("from AbstractExpression import *\n")
        file.write("from d_non_terminal import *\n\n")
        for terminal in self.terminals:
            if not terminal.isalnum():
                terminal = self.create_class_nonal_interpreter(terminal)
            terminal = terminal + "_c"
            file.write("class " + terminal + "(TerminalExpression):\n")
            file.write("\tdef interprets(self, base, childs, context):\n")
            file.write("\t\tif context.GetActualContext() == base:\n")
            file.write("\t\t\tcontext.TakeActualContext()\n")
            file.write("\t\t\tif len(childs) > 0:\n")
